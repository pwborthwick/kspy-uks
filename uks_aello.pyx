...
def aello(atom, basis, mode = 'scf', density = None, gauge = None):
    ...
    for p in range(0, nb):
        ...
        for q in range(0, len(basis[p].coefficients)):
    ...
    if mode == 'dipole':
        return aelloDipole(alo_n, alo_c, alo_e, alo_a, alo_o, alo, alo_z, alo_x, na, nb, atom, density, gauge)
    elif mode == 'angular':
        return aelloAngular(alo_n, alo_c, alo_e, alo_a, alo_o, alo, alo_z, alo_x, na, nb, atom, gauge)
    elif mode == 'nabla':
        return   aelloNabla(alo_n, alo_c, alo_e, alo_a, alo_o, alo, alo_z, alo_x, na, nb, atom)
...
cpdef aelloDipole(double[:,:] alo_n, double[:,:] alo_c, double[:,:] alo_e, short[:,:] alo_a, double[:,:] alo_o, \
                        short[:] alo, short[:] alo_z, double[:,:] alo_x, int na, int nb, object molAtom, double[:,:] density, gauge): 
